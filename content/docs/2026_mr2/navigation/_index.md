...
*Written by Jaeuk Kim in 2026. 02*
...
In this section, I will go through the major and peripheral system implementations and their wirings.
This could be either in software-level, sensor-level, or conceptual level.

Whenever I omit an implemnetation, it means either of following:
1. I was lazy.
2. I am planning to create a separate page dedicated to that topic.
...
2. You need to create a path connecting two points. (Planning)
...
(There is another LTP frame called **`utm`**; Where the map frame can be set with any datum we prefer, `utm` has a fixed convention for its datum. We also use this frame for the base-side `map` frame monitoring, but this is not an important component for now.)

By informing the GNSS with real-time kinematics (RTK), we can estimate the rover's position with centimeter accuracy.
...
While GNSS can give us absolute position and yaw, we should also estimate our position with other differential sensors.
...
The whole procedure is done by **dual-EKF** setup. I omit the detail for now.
It is highly advised understand dual-EKF setup to understand the relation between `map`, `odom`, and `base_link`.

## Planner (Nav2)

Now that we know where we are, we can plan a path to the goal.
We want this path to be safe, reflective to the obstacles, and efficient.
If you have tried studying algorithms, you might have heard of a search algorithm called **A\***, a variation of uniform cost search but with informed heuristcic, such as Eucledian distance. In our implementation, we go one step further and use **Lazy Theta\*** for taut path; Basically, we want the rover to travel in straight lines, following line-of-sight logic, and not constrained to grids. This is a considerate choice since 1. Utah desert is believed to be mostly empty 2. Empirically, it is more temporally stable than other grid-based planners.
...
The controller gives linear velocity and angular velocity for the rover to dictate rover's trajectory.
...
However, this means the rover will be only aware of the global path, and less robust to local obstacles.
So this is only good when the rover can fully trust the path planner, which may not always be the case due to frequency mismatch.

MPPI is more self-govenring but costly. With numerous cost functions, it samples hundreads of trajectories to find the optimum in a stochastic manner.
The cost function include these metrics:
 - pose deviation from the path
 - obstacle cost
 - goal orientation deviation
 - going backwards
 - ...
It works well, and does not require shim rotation guards (in-place rotation for edge cases where other controller fails).
...
Aside that fact, global and local costmap works equivalently; It also subscribes to traversability (local occupancy) data, and updates itself with new information. Both costmaps finish the layer composition with inflation.
...
Inflation results in bloating the obstacle from its original occupancy. Inflating an obstacle has two purpose:
1. Guide the planner to avoid proximity from the obstacles, but more importantly and primarily:
2. Add a patch of rover radius so the rover can be considered as a dot.

The second property is the most important aspect when you try to tune the inflation of the costmap.
*Planners does not, and cannot consider rover geometry on its own.*
In our implementation, the rover is considered as a circle with diameter of 1.2 m.
...
Here I will omit the detail on the process to acquire this slope map and how it is aligned with correct datum.
...
Here I will omit the detail on the process on the traversability pipeline for now.
...
# Behavior Trees and Action Servers: Governing Over Navigation Pipeline